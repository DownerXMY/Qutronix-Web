...
export default {
  ...
  methods: {
    changeMode() {
        ...
        this.varphiUpdate3 = "";
        this.alphaUpdate3 = "";
        for (var index=0;index<this.chosenList3.length;index++) {
            this.chosenList3[index].attr('body/fill','#af9bff');
            this.chosenList3[index].attr('label/text','');
        };
        this.chosenList3 = [];
        console.log(this.alphaUpdate3);
        console.log(this.varphiUpdate3);
        console.log(this.alpha3);
        console.log(this.varphi3);
    },
    backToBS() {
        this.visible = false;
        this.$emit("refreshDrawData2");
    },
    ...
    //   console.log(this.nodeNumber);
      ...
      const half = (this.nodeNumber-1) / 2;
      for (var number = 1; number <= half; number++) {
        let Nlink1 = new joint.shapes.standard.Link({
          source: { x: 360-per*this.nodeNumber/2, y: per*(2*number-1) },
          target: { x: 360+per*(this.nodeNumber)/2-per*(2*number-1), y: per*this.nodeNumber}
        });
        let Nlink2 = new joint.shapes.standard.Link({
          source: { x: 360-per*this.nodeNumber/2, y: 2*per*number },
          target: { x: 360-per*this.nodeNumber/2+per*(2*number-1), y: per }
        });
      ...
      for (var number1 =1; number1<=half; number1++) {
        let Nlink3 = new joint.shapes.standard.Link({
          source: { x: 360-per*this.nodeNumber/2+2*per*number1, y: per },
          target: { x: 360+per*this.nodeNumber/2, y: per*this.nodeNumber-per*(2*number1-1) }
        });
        let Nlink4 = new joint.shapes.standard.Link({
          source: { x: 360-per*this.nodeNumber/2+per*(2*number1+1), y: per*this.nodeNumber },
          target: { x: 360+per*this.nodeNumber/2, y: per*(2*number1+1) }
        });
      ...
      for (var number2 = 1; number2 <= this.nodeNumber; number2++) {
        if (number2 == 1) {
          ...
          source: { x: 360+per*(this.nodeNumber)/2, y: per*number2 },
          target: { x: 750, y: per*number2 }
          });
          let Nlink6 = new joint.shapes.standard.Link({
          source: { x: 20, y: per*number2 },
          target: { x: 360-per*this.nodeNumber/2, y: per*number2 }
          });
        ...
        } else if (number2 == this.nodeNumber) {
          let Nlink5 = new joint.shapes.standard.Link({
          source: { x: 360+per*(this.nodeNumber-2)/2, y: per*number2 },
          target: { x: 750, y: per*number2 }
          });
          let Nlink6 = new joint.shapes.standard.Link({
          source: { x: 20, y: per*number2 },
          target: { x: 360-per*(this.nodeNumber-2)/2, y: per*number2 }
          });
          Nlink5.attr('line/stroke', '#e66f2b');
          Nlink6.attr('line/stroke', '#e66f2b');
          Nlink6.labels([{
      ...
      for (var number3 = 1; number3 <= half; number3++) {
        let Nlink6 = new joint.shapes.standard.Link({
          source: { x: 360-per*this.nodeNumber/2+per*(2*number3-1), y: per },
          target: { x: 360-per*this.nodeNumber/2+per*2*number3, y: per }
        });
        Nlink6.attr('line/stroke', '#e66f2b');
        constGraph.addCells([Nlink6]);
      }
      for (var number3 = 1; number3 <= half-1; number3++) {
        let Nlink7 = new joint.shapes.standard.Link({
          source: { x: 360-per*this.nodeNumber/2+per*2*number3, y: per*this.nodeNumber },
          target: { x: 360-per*this.nodeNumber/2+per*(2*number3+1), y: per*this.nodeNumber }
        });
        Nlink7.attr('line/stroke', '#e66f2b');
        constGraph.addCells([Nlink7]);
      }

        let Nlink8 = new joint.shapes.standard.Link({
          source: { x: 360-per*(this.nodeNumber-2)/2, y: per*this.nodeNumber },
          target: { x: 360+per*(this.nodeNumber)/2, y: per }
        });
        Nlink8.attr('line/stroke', '#e66f2b');
        constGraph.addCells([Nlink8]);

      // Add choosing points
      for (var i1 = 1; i1 <= half+1; i1++) {
        for (var i2 = 1; i2 <= half; i2++) {
          var NButton = new joint.shapes.standard.Circle({
            position: {
                x: 360-per*this.nodeNumber/2+per/2+2*per*(i1-1),
      ...
      for (var i3 = 1; i3 <= half; i3++) {
        for (var i4 = 1; i4 <= half; i4++) {
          var NButton2 = new joint.shapes.standard.Circle({
            position: {
                x: 360-per*this.nodeNumber/2+per*3/2+2*per*(i3-1),
          ...
          constGraph.addCells([NButton2]);
        }
      }
      // -----------------------------------------
      // -----------------------------------------

      var chosenPoint3 = Object();
      constPaper.on('cell:pointerdown', (cellView, e, x, y) => {
        this.chosenPoint3 = cellView.model;
        this.chosenList3 = this.chosenList3.concat([this.chosenPoint3]);
        console.log(this.chosenPoint3);
      });
      ...
      $("#makeSure").on("click", () => {
        (this.alpha3 = this.alphaUpdate3),
        (this.varphi3 = this.varphiUpdate3),
        (this.chosenPoint3.attributes.attrs.label.text = "        "+this.alpha3+","+this.varphi3),
        (this.chosenPoint3.attr('body/fill','#df4617'))
      });
