...
export default {
  ...
  methods: {
    changeMode() {
        ...
        this.varphiUpdate2 = "";
        this.alphaUpdate2 = "";
        for (var index=0;index<this.chosenList2.length;index++) {
            this.chosenList2[index].attr('body/fill','#af9bff');
            this.chosenList2[index].attr('label/text','');
        };
        this.chosenList2 = [];
        console.log(this.alphaUpdate2);
        console.log(this.varphiUpdate2);
        console.log(this.alpha2);
        console.log(this.varphi2);
    },
    backToBS() {
        this.visible = false;
    ...
    inti(nodes) {
      ...
      for (var number1 =1; number1<=half-1; number1++) {
        let Nlink3 = new joint.shapes.standard.Link({
          ...
          target: { x: 360+per*(this.nodeNumber+1)*0.5, y: per*this.nodeNumber-per*(2*number1-1) }
        });
        let Nlink4 = new joint.shapes.standard.Link({
          source: { x: 360-per*(this.nodeNumber-1)/2+2*per*number1, y: per*this.nodeNumber },
          target: { x: 360+per*(this.nodeNumber+1)*0.5, y: 2*per*number1 }
        });
      ...
      for (var number2 = 1; number2 <= this.nodeNumber; number2++) {
        if (number2 == 1 || number2 == this.nodeNumber) {
          ...
          let Nlink5 = new joint.shapes.standard.Link({
          source: { x: 360+per*(this.nodeNumber+1)*0.5, y: per*number2 },
          target: { x: 750, y: per*number2 }
          });
          let Nlink6 = new joint.shapes.standard.Link({
          source: { x: 20, y: per*number2 },
      ...
      var chosenPoint2 = Object();
      constPaper.on('cell:pointerdown', (cellView, e, x, y) => {
        this.chosenPoint2 = cellView.model;
        this.chosenList2 = this.chosenList2.concat([this.chosenPoint2]);
        console.log(this.chosenPoint2);
      });
      ...
      $("#makeSure").on("click", () => {
        (this.alpha2 = this.alphaUpdate2),
        (this.varphi2 = this.varphiUpdate2),
        (this.chosenPoint2.attributes.attrs.label.text = "        "+this.alpha2+","+this.varphi2),
        (this.chosenPoint2.attr('body/fill','#df4617'))
      });
